...
class PositionWarehouse
{
    ...
    function get_pos_in_category($category, $subcategory = 0, $version = 0, $archive = 0)
    {
        ...
        $query = "SELECT * FROM pos_items_warehouse WHERE category='$category'" . $where;
        $result = $this->pdo->query($query);
        while ($line = $result->fetch()) {
            $pos_array[] = $line;
        }
    ...
    {
        $query = "SELECT * FROM pos_items_warehouse WHERE id='$id'";
        $result = $this->pdo->query($query);
        while ($line = $result->fetch()) {
            $pos_array[] = $line;
        }
    ...
    function edit_pos($id, $title, $longtitle, $category, $subcategory, $vendorcode, $provider, $price, $quant_robot, $quant_total, $min_balance, $assembly, $summary, $archive)
    {
        $date    = date("Y-m-d H:i:s");
        $user_id = intval($_COOKIE['id']);
        $query   = "UPDATE `pos_items_warehouse` SET `title` = '$title', `longtitle` = '$longtitle', `category` = '$category', `subcategory` = '$subcategory', `provider` = '$provider', `price` = '$price', `quant_robot` = '$quant_robot', `total` = '$quant_total', `min_balance` = '$min_balance', `vendor_code` = '$vendorcode', `assembly` = '$assembly', `summary` = '$summary', `archive` = '$archive', `update_date` = '$date', `update_user` = '$user_id' WHERE `pos_items_warehouse`.`id` = $id;";
        $result = $this->pdo->query($query);
    ...
    function del_pos($id)
    {
        //$date    = date("Y-m-d H:i:s");
        //$user_id = intval($_COOKIE['id']);
        $query   = "DELETE FROM `pos_items_warehouse` WHERE `pos_items_warehouse`.`id` = $id";
        $result = $this->pdo->query($query);
    ...
    function from_warehouse($pos_id)
    {
        ...
        if (isset($pos)) {
            ...
            $query   = "INSERT INTO `pos_items` (`id`, `category`, `subcategory`,`title`, `vendor_code`, `provider`, `price`, `longtitle`, `version`, `quant_robot`, `total`, `reserv`, `assembly`, `summary`,`apply`, `ow`, `min_balance`, `img`, `archive`, `update_date`, `update_user` ) VALUES ('$pos_id', '$category', '$subcategory', '$title', '$vendorcode', '$provider', '$price', '$longtitle', '$version', '$quant_robot', '$total', '$reserv', '$assembly', '$summary', '$apply', '$ow', '$min_balance', '$img', '$archive', '$date', '$user_id')";
            $result = $this->pdo->query($query);

            if ($result) {
                $this->del_pos($pos_id);
                $this->log->add(__METHOD__,"Создана новая позиция на основном складе $vendorcode $title через перемещение");
                return true;
            }
        }
